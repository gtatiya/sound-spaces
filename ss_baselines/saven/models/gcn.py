...
class DGL_GCN(nn.Module):
    def __init__(self, in_feats, o_feats, debug=False):
        ...
        objects = list(sorted(objects_vector.keys())) # 21 types of object: ['bathtub', 'bed', ..., 'towel', 'tv_monitor']
        regions = list(sorted(regions_vector.keys())) # 24 types of room: ['balcony', 'bathroom', ..., 'utilityroom/toolroom', 'workout/gym/exercise']
        self.n = len(objects) + len(regions) 
        if self.debug:
        ...
        self.all_glove = all_glove
        dim_rep = 512
        self.get_word_embed = nn.Linear(300, dim_rep)
        self.get_img_embed = nn.Linear(1000, dim_rep)
        ...
        self.final_mapping = nn.Linear(self.n, dim_rep)

    def forward(self, class_embed):
        '''
        class_embed should be 1000 dim
        '''
...
if __name__ == '__main__':
    model = DGL_GCN(1024, 1, debug=True)
    joint_rep = torch.rand(1000)
